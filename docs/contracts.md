...
Deployed: StudioNet `0x7e7544B55d0d905286C2eb7E6389Acd6522aCE4A`
...
- **Added a reproducible test suite** — `tests/test_contract_static.py` (source-level safety/regression checks, run via plain Python, no pytest required) and `tests/test_lifecycle_model.py` (a pure-Python state-machine model of the full lifecycle: lock_return, condition checks, challenges — upheld/overturned/rejected — voided outcomes, and finalization). Both were actually run and confirmed passing (26/26 combined) before this update, not merely written. Neither is a live-consensus test — see `docs/live-verification.md` for that.
- **`resolve_check`'s render-failure handling now surfaces the real exception instead of a generic message.** The prior version's `except Exception: return {"void_reason_code": "SOURCE_UNAVAILABLE", ...}` discarded the actual error, making a genuine network failure indistinguishable from a URL that resolves fine but isn't renderable. Now stores a sanitized `f"{type(exc).__name__}: {exc}"` (capped at 200 chars) in `reasoning_summary`. This is what let live testing correctly diagnose that `gl.nondet.web.render(mode="screenshot")` needs a real HTML page, not a bare image URL — see `docs/deployment.md`'s Evidence Hosting Requirements section. Does not change validator agreement logic: both jurors still hit the same exception independently and both still produce the same void path.
