    ...
    <div className="container detail-page">
      ...
      <dl className="record" style={{ marginTop: 24 }}>
        <div className="record-row">
          <dt>Category</dt>
          <dd>{rental.item_category}</dd>
        </div>
        <div className="record-row">
          <dt>Owner</dt>
          <dd className="mono">{shortAddress(rental.owner)}</dd>
        </div>
        <div className="record-row">
          <dt>Renter</dt>
          <dd className="mono">{shortAddress(rental.renter)}</dd>
        </div>
        <div className="record-row">
          <dt>Reference locked</dt>
          <dd>{formatEpoch(rental.reference_locked_at)}</dd>
        </div>
        {rental.return_locked_at > 0 && (
          <div className="record-row">
            <dt>Return locked</dt>
            <dd>{formatEpoch(rental.return_locked_at)}</dd>
          </div>
        )}
        <div className="record-row">
          <dt>Description</dt>
          <dd>{rental.item_description}</dd>
        </div>
      </dl>

      {/* Step: renter locks return evidence */}
      {rental.status === 'OPEN' && isRenter && (
        <section className="action-block">
          ...
          <div className="field">
            <label htmlFor="return-url">Return photo URL</label>
            <input
              id="return-url"
              value={returnUrl}
          ...
          {returnCommitment && (
            <div className="commitment-preview">
              <span className="hint">Commitment:</span>
              <code className="mono">{returnCommitment}</code>
            </div>
          )}
          <button
            className="btn btn--primary"
            disabled={busy !== null || !isValidEvidenceUrl(returnUrl)}
            onClick={() =>
              runAction('lock_return', () =>
                writeContract('lock_return', [rental.rental_id, returnUrl.trim(), returnCommitment])
              )
            }
          >
      ...
      {rental.status === 'OPEN' && !isRenter && (
        <div className="empty-state">Waiting for the renter to lock return evidence.</div>
      )}

      {/* Step: file a condition check */}
      {rental.status === 'RETURNED' && isParty && (
        <section className="action-block">
          ...
          <button
            className="btn btn--primary"
            disabled={busy !== null}
            onClick={() => runAction('file_condition_check', () => writeContract('file_condition_check', [rental.rental_id]))}
          >
      ...
      {check && check.status === 'filed' && (
        <section className="action-block">
          ...
          <button
            ...
            disabled={busy !== null}
            onClick={() => runAction('resolve_check', () => writeContract('resolve_check', [check.check_id]))}
          >
      ...
      {/* Escrowed verdict — challenge window, or finalize */}
        ...
        <section className="action-block">
          ...
          {check.reason_codes.length > 0 && (
            <ul className="reason-list">
              {check.reason_codes.map((rc) => (
                <li key={rc}>{REASON_LABEL[rc] ?? rc}</li>
              ))}
            </ul>
          )}
          ...
          {isParty && (
            <div className="challenge-form">
              ...
              <div className="field">
                ...
                <select id="reason" value={challengeReason} onChange={(e) => setChallengeReason(e.target.value)}>
                  {CHALLENGE_REASON_CODES.map((rc) => (
              ...
              <div className="field">
                ...
                <textarea
                  ...
                  placeholder="Explain specifically what the jury got wrong."
                />
              </div>
              <button
                className="btn"
                disabled={busy !== null || challengeStatement.trim().length < 10}
                onClick={() =>
                  runAction('open_challenge', () =>
                    writeContract('open_challenge', [check.check_id, challengeReason, challengeStatement.trim()])
                  )
                }
              >
                {busy === 'open_challenge' ? 'Filing challenge…' : 'File challenge'}
              </button>
            </div>
          )}
          ...
          <button
            className="btn btn--primary"
            disabled={busy !== null}
            onClick={() => runAction('finalize_check', () => writeContract('finalize_check', [check.check_id]))}
          >
            {busy === 'finalize_check' ? 'Finalizing…' : 'Finalize (after window closes)'}
          </button>
        </section>
      )}

      {/* Challenged — resolve the challenge */}
      {check && check.status === 'challenged' && challenge && (
        <section className="action-block">
          ...
          <dl className="record">
            <div className="record-row">
              <dt>Reason</dt>
              <dd>{challenge.reason_code.replaceAll('_', ' ').toLowerCase()}</dd>
            </div>
            <div className="record-row">
              <dt>Statement</dt>
              <dd>{challenge.statement}</dd>
            </div>
          </dl>
          <button
            className="btn btn--sage"
            disabled={busy !== null}
            onClick={() => runAction('resolve_challenge', () => writeContract('resolve_challenge', [challenge.challenge_id]))}
          >
            {busy === 'resolve_challenge' ? 'Re-adjudicating…' : 'Resolve challenge'}
          </button>
        </section>
      )}

      {check && check.status === 'finalized' && (
        <div className="empty-state">
          <p style={{ margin: '0 0 12px' }}>
            This condition check is finalized. The verdict is now part of both parties' permanent
            record.
          </p>
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
            <Link to={`/reputation?address=${rental.owner}`} className="btn">
              View owner's record
            </Link>
            <Link to={`/reputation?address=${rental.renter}`} className="btn">
              View renter's record
            </Link>
          </div>
        </div>
      )}

      {check && check.status === 'voided' && (
        <div className="empty-state">
          <p>{check.reasoning_summary}</p>
        </div>
      )}
    </div>
  );
